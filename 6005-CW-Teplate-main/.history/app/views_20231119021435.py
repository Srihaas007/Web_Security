...
def create():
    ...
    password = flask.request.form.get("password")
    Check_password = flask.request.form.get("password2")
    terms = flask.request.form.get("terms")


    # inside your create() function
    password = flask.request.form.get("password").encode('utf-8')
    ...
    if not email or not password or not Check_password: 
        ...
        flask.flash("The password should contain atleast 8 char which includes 1 special,1 Upper, 1 numerical values ")
        return flask.render_template("create_account.html",
                                     email = email)
        ...
        flask.flash("Please read the terms and conditions")
        return flask.render_template("create_account.html",
                                     email = email)

    else:#Otherwise we can add the user
