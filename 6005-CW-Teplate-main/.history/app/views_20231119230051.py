...
def reviewItem(userId, itemId):
    ...
    if flask.request.method == "POST":
        ...
        reviewComment = flask.request.form.get("review").strip()
        reviewId = flask.request.form.get("reviewId")

        # Process the uploaded file
        file = flask.request.files.get('review_image')
        upload_folder_path = r'C:/Users/sriha\Desktop/6005-CW-Teplate-main (1)/6005-CW-Teplate-main/app/uploads'
        ...
        if file and allowed_file(file.filename):
            ...
            # Use filename or file_path depending on how you want to store it
            image_path = filename  # or file_path for full path
        else:
            image_path = None
        ...
        if reviewId:
            ...
            # Insert a new review
            theSQL = f"""INSERT INTO review (userId, productId, stars, review, image)
                         VALUES ({userId}, {itemId}, {reviewStars}, '{reviewComment}', '{image_path}');"""

        write_db(theSQL)

        flask.flash("Review Updated" if reviewId else "Review Made")

    # Fetch the product and review information
    item = query_db(f"SELECT * FROM product WHERE id = {itemId};", one=True)
    review = query_db(f"SELECT * FROM review WHERE userID = {userId} AND productID = {itemId};", one=True)
