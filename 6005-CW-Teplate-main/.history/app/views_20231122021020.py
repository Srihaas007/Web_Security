...
def seller():
    ...
    if request.method == "POST":
        ...
        user_email = request.form.get("email")
        user_password = request.form.get("password").encode('utf-8')
        app.logger.info(f"Attempt to register as seller with email: {user_email}")
        ...
        if userQry is None:
            ...
            if bcrypt.checkpw(user_password, stored_password_hash):
                app.logger.info(f"Login as {user_email} successful")
                # Now, create a new seller with the same email and password
                seller_email = user_email
                seller_password = stored_password_hash  # Use the hashed password from the user
