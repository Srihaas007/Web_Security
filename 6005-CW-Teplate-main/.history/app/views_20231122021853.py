...
@app.route("/user/newseller_registration", methods=["GET", "POST"])
def seller():
    """
    ...
    if request.method == "POST":
        ...
        if user_id:
            ...
            user = query_db(user_query, [user_id], one=True)

            if user:
                seller_email = user["email"]
                # You can use the same password or hash from the user table if needed
                seller_password = user["password"]  # Assuming this is the hashed password from the user table

                # Insert the seller data into the seller table
                insert_query = "INSERT INTO seller (email, password) VALUES (?, ?)"
                db = get_db()  # Get your database connection
                db.execute(insert_query, (seller_email, seller_password))
                db.commit()
                flash("Seller registration successful")
                return redirect(url_for("index"))
            else:
    ...
    return render_template("newseller_registration.html")
