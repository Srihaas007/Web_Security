...
@app.route("/user/become_seller", methods=["GET", "POST"])
def become_seller():
    """
    ...
    if request.method == "POST":
        ...
        if user_id:
            ...
            if user:
                ...
                seller_query = "SELECT * FROM seller WHERE email = ?"
                existing_seller = query_db(seller_query, [user_email], one=True)

                if existing_seller:
                    flash("You are already a seller.")
                else:
                    # Create a new seller account with the same email
                    insert_query = "INSERT INTO seller (email) VALUES (?)"
                    db = get_db()  # Get your database connection
                    db.execute(insert_query, [user_email])
                    db.commit()
