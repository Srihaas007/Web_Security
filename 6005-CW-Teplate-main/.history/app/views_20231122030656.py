    ...
    if request.method == "POST":
        ...
        try:
            ...
            if existing_seller:
                ...
                # Add the email to the seller table in your SQLite database
                insert_query = "INSERT INTO seller (email) VALUES (?)"
                cursor.execute(insert_query, (email,))

                # Copy user information to seller table
                user_query = "SELECT * FROM user WHERE email = ?"
                cursor.execute(user_query, (email,))
                user_data = cursor.fetchone()

                if user_data:
                    # Insert user data into the seller table (modify column names as needed)
                    insert_user_query = "INSERT INTO seller (email, password) VALUES (?, ?)"
                    cursor.execute(insert_user_query, (user_data["email"], user_data["password"]))
