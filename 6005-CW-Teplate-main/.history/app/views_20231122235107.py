...
def add_products():
    if not session.get("is_seller"):
        flash("You need to be a seller to add products.")
        return redirect(url_for("index"))
    ...
    if request.method == "POST":
        ...
        if not name or not description or not price or not image:
            ...
            insert_query = "INSERT INTO product (name, description, price, image) VALUES (?, ?, ?, ?)"
            write_db(insert_query, (name, description, price, filename))
...
def edit_products():
    ...
    # Fetch the products associated with the seller's user ID from the product table
