...
def login():
    ...
    if request.method == "POST":
        ...
        app.logger.info(f"Attempt to login as {email}")

        # Check if the user is an admin
        admin_query = "SELECT * FROM Admin WHERE email = ?"
        admin = query_db(admin_query, [email], one=True)

        if admin:
            # This is an admin login
            app.logger.info("Admin found")
            stored_password_hash = admin["password"]  # Assuming this is the hashed password from the database
            if bcrypt.checkpw(password, stored_password_hash):
                app.logger.info(f"Login as admin {email} successful")
            ...
            if user:
                ...
                stored_password_hash = user["password"]  # Assuming this is the hashed password from the database
                if bcrypt.checkpw(password, stored_password_hash):
                    app.logger.info(f"Login as user {email} successful")
