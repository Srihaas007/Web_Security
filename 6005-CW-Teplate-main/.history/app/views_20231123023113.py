...
def login():
    ...
    if flask.request.method == "POST":
        ...
        theQry2 = "Select * FROM admin WHERE email = '{0}'".format(user)

        userQry2 =  query_db(theQry2, one=True)
        if userQry2 :
            app.logger.info("admin is Ok")
            admin_password= userQry2["password"]
            
            if (password==admin_password):
                app.logger.info("Login as %s Success", userQry2["email"])
                flask.session["user"] = userQry2["id"]
                flask.flash("ADminLogin Successful")
                return flask.render_template("admin_dashboard.hml")
            
            

        elif userQry is None:
