...
def admin_login():
    ...
    if flask.request.method == "POST":
        # Get data
        admin_email = flask.request.form.get("email")
        password = flask.request.form.get("password")
        app.logger.info("Attempt to login as admin %s:%s", admin_email, password)

        # Check if the provided email belongs to an admin
        admin_query = "SELECT * FROM admin WHERE email = ?"
        admin_data = query_db(admin_query, [admin_email], one=True)
        ...
        flask.flash("Admin Login Failed")
...
def user_login():
    ...
    if flask.request.method == "POST":
        ...
        password = flask.request.form.get("password")
        app.logger.info("Attempt to login as user %s:%s", user_email, password)

        # Check if the provided email belongs to a regular user
