...
def settings(userId):
    ...
    #Purchases
    theSQL = "Select * FROM purchase WHERE userID = {userId}"
    ...
    if flask.request.method == "POST":
        ...
        if current:
            if current == thisUser["password"]:
                ...
                #Update the Password
                theSQL = "UPDATE user SET password = '{password}' WHERE id = {userId}"
...
def reviewItem(userId, itemId):
    ...
    # Fetch the review
    product_query = "SELECT * FROM product WHERE id = {itemId};"
    item = query_db(product_query, one=True)
    
    review_query = "SELECT * FROM review WHERE userID = {userId} AND productID = {itemId};"
...
def basket():
    ...
    for key in sessionBasket:
        theQry = "SELECT * FROM product WHERE id = {key}"
...
def pay():
    ...
    theQry = "Select * FROM User WHERE id = ?".format(flask.session["user"])
