...
def settings(userId):
    ...
    try:
        ...
        # Execute the SQL query for purchases
        theSQL = "SELECT * FROM purchase WHERE userID = ?"
        cursor.execute(theSQL, (userId,))
        purchases = cursor.fetchall()

        # Initialize a dictionary to store whether a review has been submitted for each product
        reviews_submitted = {}

        # Initialize a list to store purchased products with details
        ...
        for purchase in purchases:
            ...
            # Check if a review has been submitted by the user for this product
            review_query = "SELECT * FROM reviews WHERE userID = ? AND productID = ?"
            cursor.execute(review_query, (userId, product_id))
            review = cursor.fetchone()

            # If a review exists, set the flag to True; otherwise, set it to False
            reviews_submitted[product_id] = review is not None

            # Fetch product details for the purchased product
            product_details_query = """
            SELECT id, name, description  -- Adjust the columns you need
            FROM product
            WHERE id = ?;
            """
            ...
            if product_details:
                purchases_with_details.append(product_details)
    ...
    return flask.render_template("usersettings.html",
                                 user=thisUser,
                                 purchases=purchases,
                                 purchases_with_details=purchases_with_details,
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            if reviewId:
                ...
                WHERE id = ?
                """
                cursor.execute(update_query, (reviewStars, reviewComment, filename, reviewId, image_path))
