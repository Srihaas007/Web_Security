...
import datetime
import bcrypt
from flask import flash, request,redirect,session, render_template, request, url_for,g
...
UPLOAD_FOLDER = 'C:/Users/sriha/OneDrive/Desktop/6005-CW-Teplate-main (1)/uploads'
...
def login():
    ...
    if flask.request.method == "POST":
        ...
        user_query2 = "SELECT * FROM seller WHERE email = ?"
        user_data2 = query_db(user_query2, [user_email], one=True)
        ...
        if user_data:
            ...
            if check_password(password, hashed_db_password):
                ...
                if user_data2:
                    flask.session["is_seller"]=flask.session["user"]
...
def search_product():
    ...
    # Use SQL's LIKE operator to find a match in the product table
    ...
    if products:
        # Render the product.html with the first matching product
        # You might want to allow the user to select from all matches or refine this further
        return render_template('product.html', item=products[0], reviews=[])  # Include reviews if applicable
    else:
        flash("No products found for your search.")
        return redirect(url_for('index'))  # Redirect to some page if no product is found
    ...
    if flask.request.method == "POST":
        ...
        if current:
            if current == thisUser["password"]:
                ...
                cursor.execute(theSQL, (password, userId))
    ...
    if request.method == "POST":
        ...
        with get_db() as conn:
            ...
            if reviewId:
                cursor.execute("""
                    UPDATE review
                    SET stars = ?, review = ?, image_path = ?
                    WHERE id = ?
                """, (reviewStars, reviewComment, filename, reviewId))
                flash("Review Updated")
            else:
                cursor.execute("""
                    INSERT INTO review (userId, productId, stars, review, image_path)
                    VALUES (?, ?, ?, ?, ?)
                """, (userId, itemId, reviewStars, reviewComment, filename))
                flash("Review Added")
            conn.commit()

    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM product WHERE id = ?", (itemId,))
        item = cursor.fetchone()
        cursor.execute("SELECT * FROM review WHERE userID = ? AND productID = ?", (userId, itemId))
...
def pay():
    ...
    for key in sessionBasket:
        ...
        values = (theUser['id'], key, theDate)
        
        # Execute the query on the database connection
        write_db(theQry, values)
